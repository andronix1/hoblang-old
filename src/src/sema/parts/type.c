...
SemaType *sema_ast_type(SemaModule *sema, AstType *type) {
	...
	switch (type->type) {
		case AST_TYPE_FUNCTION: {
			...
			for (size_t i = 0; i < vec_len(type->func_type.args); i++) {
				SemaType* arg_type = sema_ast_type(sema, &type->func_type.args[i]);
				if (arg_type) {
					return NULL;
				}
			...
			stype->func.returning = sema_ast_type(sema, type->func_type.returns);
			type->sema = stype;
			break;
		}
