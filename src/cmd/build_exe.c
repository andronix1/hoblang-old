...
void cmd_usage_build_exe() {
    ...
    CMD_USAGE_FLAG("--link-arg <flag>", "pass flag to linker");
...
bool cmd_parse_build_exe(Cmd *output, const char **args, size_t len) {
    ...
    CMD_PARSE {
        ...
        if (CMD_IS_FLAG) {
            ...
            CMD_FLAG_APPEND("link-arg", output->build_exe.linker.args);
            ...
            CMD_LLVM_FLAGS(output->build_exe.llvm);
            CMD_FLAGS_END;
        } else {
