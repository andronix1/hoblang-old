...
#include "sema/nodes/decl/external.h"
    ...
    switch (decl->kind) {
        case AST_NODE_DECL_GENERIC: {
            ...
            if (is_generic) {
                ...
                sema_module_push_decl(sema, decl->decl_loc, sema_decl_new(
        ...
        case AST_NODE_DECL_EXTERNAL: sema_module_read_external(sema, &decl->external, decl->public); break;
        ...
        case AST_NODE_DECL_GENERIC:
            switch (decl->generic.kind) {
                case AST_NODE_DECL_GENERIC_FUNC: {
                    AstFuncDecl *func = &decl->generic.func_decl;
                    ...
                    if (func->info->ext_of) {
                        ...
                        if (type) {
                            sema_module_push_decl(sema, func->info->name_loc, sema_decl_new(
                                slice_from_cstr("self"),
                                sema_value_new_runtime(
                                    sema_value_runtime_new_var(type))), false);
                        }
                    }
                    for (size_t i = 0; i < vec_len(func->info->args); i++) {
                        ...
                        sema_module_push_decl(sema, arg->name_loc, sema_decl_new(arg->name,
                            sema_value_new_runtime(sema_value_runtime_new_var(type))), false);
                    }
