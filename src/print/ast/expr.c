...
void print_ast_binop_type(FILE *stream, va_list list) {
    ...
    switch (binop) {
        case AST_BINOP_ADD: print_to(stream, "+"); break;
        case AST_BINOP_SUB: print_to(stream, "-"); break;
        case AST_BINOP_MUL: print_to(stream, "*"); break;
        case AST_BINOP_BITAND: print_to(stream, "&"); break;
        case AST_BINOP_BITOR: print_to(stream, "|"); break;
        case AST_BINOP_SHR: print_to(stream, ">>"); break;
        case AST_BINOP_SHL: print_to(stream, "<<"); break;
        case AST_BINOP_XOR: print_to(stream, "^"); break;
        case AST_BINOP_DIV: print_to(stream, "/"); break;
        case AST_BINOP_EQ: print_to(stream, "=="); break;
        case AST_BINOP_NEQ: print_to(stream, "!="); break;
        case AST_BINOP_GT: print_to(stream, ">"); break;
        case AST_BINOP_GE: print_to(stream, ">="); break;
        case AST_BINOP_LT: print_to(stream, "<"); break;
        case AST_BINOP_LE: print_to(stream, "<="); break;
        case AST_BINOP_AND: print_to(stream, "&&"); break;
        case AST_BINOP_OR: print_to(stream, "||"); break;
        case AST_BINOP_MOD: print_to(stream, "%"); break;
	}
}
...
void print_ast_expr(FILE *stream, va_list list) {
	...
	switch (expr->type) {
		case AST_EXPR_ANON_FUN: {
            print_to(stream, "fun (");
            for (size_t i = 0; i < vec_len(expr->anon_fun.info.args); i++) {
                AstFuncArg *arg = &expr->anon_fun.info.args[i];
                if (i != 0) {        
                    fprintf(stream, ", ");
                }
                print_to(stream, "{slice}: {ast::type}", &arg->name, arg->type);
            }
            print_to(stream, ") -> {ast::type}", expr->anon_fun.info.returning);
            fprintf(stream, "{...}");
