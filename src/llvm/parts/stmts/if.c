...
void llvm_stmt_if(LlvmBackend *llvm, AstIfElse *if_else) {
	...
	LLVMBuildCondBr(llvm->builder, llvm_expr(llvm, if_else->main.expr, true), if_block, else_block);
	...
	for (size_t i = 0; i < vec_len(if_else->else_ifs); i++) {
		...
		LLVMBuildCondBr(llvm->builder, llvm_expr(llvm, bl->expr, true), if_block, else_block);
