...
LLVMValueRef llvm_opt_value_direct(LlvmBackend *llvm, LLVMValueRef opt) {
    return LLVMBuildExtractValue(llvm_builder(llvm), opt, 1, "opt.value");
}

LLVMValueRef llvm_opt_value(LlvmBackend *llvm, LLVMTypeRef type, LLVMValueRef opt) {
    LLVMValueRef indices[2] = {
        LLVMConstInt(LLVMInt32Type(), 0, false),
        LLVMConstInt(LLVMInt32Type(), 1, false)
    };
    return LLVMBuildGEP2(llvm_builder(llvm), type, opt, indices, 2, "opt.value");
}
...
LLVMValueRef llvm_opt_wrap(LlvmBackend *llvm, LLVMTypeRef of, LLVMValueRef value) {
    ...
    LLVMBuildStore(llvm_builder(llvm), value, llvm_opt_value(llvm, opt_type, opt));
    return LLVMBuildLoad2(llvm_builder(llvm), opt_type, opt, "loaded_opt");
}
